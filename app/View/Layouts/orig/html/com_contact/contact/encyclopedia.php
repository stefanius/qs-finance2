...
<div class="contact<?php echo $this->pageclass_sfx?>">
	...
	<?php if ($this->contact->name && $this->request->params->get('show_name')) : ?>
		<h2>
	...
	<div class="encyclopedia_col2">
		...
		<?php if ($this->contact->con_position && $this->request->params->get('show_position')) : ?>
			<p class="contact-position"><?php echo $this->contact->con_position; ?></p>
		<?php endif; ?>
		<?php //Let's use state to put the family name.  ?>
		<?php if ($this->contact->state && $this->request->params->get('show_state')) : ?>
			<p class="contact-state"><?php echo $this->contact->state; ?></p>
		<?php endif; ?>
		<?php // Let's use contry to list the main countries it grows in. '?>
		<?php if ($this->contact->country && $this->request->params->get('show_country')) : ?>
			<p class="contact-country"><?php echo $this->contact->country; ?></p>
		<?php endif; ?>
	</div>
	
<div class="clr"> </div>
	<?php  if ($this->request->params->get('presentation_style')!='plain'):?>
		<?php  echo  JHtml::_($this->request->params->get('presentation_style').'.start', 'contact-slider'); ?>	
	<?php endif ?>
...
<?php echo $this->loadTemplate('links'); ?>
	...
	<?php if ($this->request->params->get('presentation_style')!='plain'):?>
			<?php echo JHtml::_($this->request->params->get('presentation_style').'.end'); ?>
			<?php endif; ?>
</div>
